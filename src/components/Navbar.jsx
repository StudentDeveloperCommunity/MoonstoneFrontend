...
const Navbar = memo(function Navbar() {
  ...
  const gradientTextHover = "hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-orange-400 hover:via-pink-500 hover:to-cyan-400 hover:drop-shadow-[0_0_10px_rgba(236,72,153,0.5)] transition-all duration-300";

  const mobileLinkStyle = "group flex items-center gap-4 p-3.5 rounded-xl text-white/90 hover:text-white hover:bg-white/5 border border-transparent hover:border-white/10 transition-all duration-300 active:scale-[0.98]";

  const mobileIconStyle = "w-5 h-5 text-white/60 group-hover:text-cyan-400 transition-colors duration-300 drop-shadow-[0_0_5px_rgba(255,255,255,0.2)]";
    ...
    <>
        ...
        <div className="relative w-full max-w-7xl px-2 sm:px-4">
          ...
          <div className="relative flex items-center justify-between px-4 sm:px-6 py-2 sm:py-3">
            ...
            <div className="hidden lg:flex items-center gap-6 xl:gap-8">
              <Link to="/" className={`text-white/90 text-[15px] font-medium tracking-wide ${gradientTextHover}`}>
                Home
              </Link>

              <Link className={`text-white/90 text-[15px] font-medium tracking-wide ${gradientTextHover}`} to="/about">
                About
              </Link>
              ...
              <Link to="/developers" className={`text-white/90 text-[15px] font-medium tracking-wide ${gradientTextHover}`}>
                Developers
              </Link>
      ...
      {isMobileOpen && (
        <>
          ...
          >
            {/* Home */}
            <Link className={mobileLinkStyle} to="/" onClick={() => setIsMobileOpen(false)}>
              <svg className={mobileIconStyle} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
              </svg>
              <span className="text-lg font-medium tracking-wide group-hover:translate-x-1 transition-transform duration-300">Home</span>
            </Link>

            {/* About */}
            <Link className={mobileLinkStyle} to="/about" onClick={() => setIsMobileOpen(false)}>
              <svg className={mobileIconStyle} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span className="text-lg font-medium tracking-wide group-hover:translate-x-1 transition-transform duration-300">About</span>
            </Link>
            ...
            >
              ...
              <span className="text-lg font-medium tracking-wide group-hover:translate-x-1 transition-transform duration-300">Events</span>
            </button>

            {/* Developers */}
            <Link className={mobileLinkStyle} to="/developers" onClick={() => setIsMobileOpen(false)}>
              <svg className={mobileIconStyle} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
              </svg>
              <span className="text-lg font-medium tracking-wide group-hover:translate-x-1 transition-transform duration-300">Developers</span>
            </Link>
