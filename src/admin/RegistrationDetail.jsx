...
export default function RegistrationDetail({ userRole }) {
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (userRole === 'admin') {
      ...
      filtered = clubFilter === "all" ? events : events.filter((event) => event.eventType === clubFilter);
    } else {
      ...
      <div className="flex gap-4 bg-gray-100 p-4 rounded-lg">
        ...
        >
          ...
          {filteredEvents.length > 0 && (
            <optgroup label={`Available ${userRole === 'admin' ? (clubFilter === "all" ? "All" : clubFilter) : userRole} Programs`}>
